def run_app():
    ...
    import pandas as pd
    from app.get_features import get_features
    ...
    with st.form("user_input_form"):
        ...
        if submitted:
            ...
            data = pd.DataFrame([user_inputs]).T.rename(columns={0: "Your Inputs"}).astype(str)
            data.index.names = ["Categories"]
            st.dataframe(data, use_container_width=True, key="user_inputs")

            st.write(f"Predicted Income:", f"**{prediction}**")
